...
Стартовые сценарии создаются сидингом (`make seed`). «Общий сбор» (`general_gathering`)
уже настроен: громкость 65%, fade-in 2s, требует подтверждения. Привяжите к нему
трек в боте (`/panel` → Сценарии) или в Mini App.
