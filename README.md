...
> ⚡ **Самый быстрый бесплатный старт (Linux/macOS/RPi):** заполните три поля в
> `.env` и выполните `make quickstart` — он поставит зависимости, создаст базу,
> засеет данные и импортирует музыку. Затем `make run-backend` и `make run-agent`.
> Для панели Mini App с телефона бесплатно: `make tunnel` (Cloudflare Tunnel).
> Старый/слабый ноутбук тянет это спокойно — БД по умолчанию SQLite (файл, без
> отдельного сервера).
